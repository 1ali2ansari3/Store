...
class ReviewController extends Controller
{
    public function add($product_slug)
    {
        ...
        if($product)
        {
            ...
            {
                $verified_pruchase = Order::where('orders.user_id',Auth::id())
                        ->join('order_items','orders.id','order_items.order_id')
                        ->where('order_items.prod_id', $product_id)->get();
            return view('frontend.reviews.index',compact('product','verified_pruchase'));
            }
        }
        else
        {
    ...
    public function create(Request $request)
    {
        ...
        if($product)
        {
            ...
            if($new_review)
            {
                return redirect('categorys/'.$category_slug.'/'.$prod_slug)->with('status', "Thank you for writing a review");
            }
        }
        else
        {
    ...
    public function update(Request $request)
    {
        ...
        if($user_review != '')
        {
            ...
            if($review)
            {
                ...
                $review->update();
                return redirect('categorys/'.$review->product->category->slug.'/'.$review->product->slug)->with('status', "Review Updated successfully");
            }
            else
            {
                return redirect()->back()->with('status', "The link you followed was broken");
